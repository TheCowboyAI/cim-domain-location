# Location Service - Leaf Node Deployment

Quick guide for deploying the Location Service on CIM leaf nodes (NixOS or nix-darwin).
...
- Hosts one or more NATS-enabled services (like location-service)
- Connects to a NATS cluster (3+ leaf nodes) for high availability
- Can connect to a super-cluster for global distribution

The location-service runs on each leaf node and processes location commands/events.
...
Add `cim-domain-location` as a flake input:
...
# flake.nix
{
  inputs = {
    ...
    cim-domain-location.url = "github:thecowboyai/cim-domain-location";
  };
...
# flake.nix
{
  ...
  outputs = { self, nixpkgs, darwin, cim-domain-location }: {
...
| `streamName` | string | `"LOCATION_EVENTS"` | JetStream stream name (must be consistent across cluster) |
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `user` | string | `"location-service"` | Service user account |
| `group` | string | `"location-service"` | Service group |
...
2. **Configure location-service** to use local NATS:
   ```nix
   services.location-service = {
     enable = true;
     natsUrl = "nats://localhost:4222";  # Still local!
     streamName = "LOCATION_EVENTS";     # Same on all leafs
   };
   ```

The location-service always connects to the local NATS server. The NATS servers handle cluster communication.
...
# flake.nix
{
  inputs = {
    nixpkgs.url = "github:NixOS/nixpkgs/nixos-unstable";
    cim-domain-location.url = "github:thecowboyai/cim-domain-location/v0.8.0";
  };
    ...
    darwinConfigurations.dev-mac = darwin.lib.darwinSystem {
      ...
      modules = [
        cim-domain-location.leafModule
        {
          ...
          # Location service with debug logging
          services.location-service = {
            enable = true;
            natsUrl = "nats://localhost:4222";
            streamName = "LOCATION_EVENTS";
...
**NixOS:**
```bash
systemctl is-active location-service
```
...
2. An event published to `events.location.{id}.defined`
...
```bash
# NixOS
journalctl -u location-service -n 50

# macOS
tail -n 50 /var/log/location-service.error.log
```
...
All three will share the same `LOCATION_EVENTS` stream across the cluster.
...
- [Location Domain README](../README.md) - Domain documentation
